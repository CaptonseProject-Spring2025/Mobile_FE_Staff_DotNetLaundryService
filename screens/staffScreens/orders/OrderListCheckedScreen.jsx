...
import { View, Text, FlatList, Alert, TouchableOpacity, Image, Modal, ScrollView } from "react-native";
...
function OrderListCheckedScreen({ navigation }) {
  ...
  useEffect(() => {
    ...
    const fetchCustomerDetails = async () => {
      const details = {};
      if (orderChecked && orderChecked.length > 0) {
        for (const order of orderChecked) {
    ...
    <View className="flex-1 bg-gray-100">
      <View className="bg-white py-4 px-4 shadow-sm">
        <Text className="text-xl font-bold text-gray-800">
          Danh sách đơn hàng đã kiểm tra
        </Text>
      </View>
